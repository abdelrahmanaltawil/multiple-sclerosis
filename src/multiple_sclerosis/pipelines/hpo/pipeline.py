...
# local imports
from multiple_sclerosis.pipelines.hpo.nodes import hyperparameters_optimization, test_best_model, study_report, study_visualization
...
def create_pipeline(**kwargs) -> Pipeline:
    return pipeline([
            ...
            node(
                func=test_best_model,
                inputs=["study", "X_train", "y_train", "X_test", "y_test", "params:neural_network", "params:data.normalize_input"],
                outputs=["performance_metrics", "trained_model"],
                name="test_best_model",
            ),
            node(
                func=study_report,
                inputs="study",
                outputs= ["best_model_parameters", "trials_dataframe"],
                name="study_report",
            ),
            node(
